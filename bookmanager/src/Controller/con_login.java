...
{	
	...
	@FXML
	public TextField Manager_IDField;//引用注册时用户输入的ID
	...
	@FXML
	public void OK() throws NumberFormatException, SQLException {//注册完毕后按钮OK单击事件
		...
		String MID = Manager_IDField.getText();//用来存放Manager ID
		...
		{
		List<Manager> manager = md.checkrepeat(Long.parseLong(MID));
		if(manager.size()!=0)
		{
			repeat();
		}
		...
		m.setPassword(PWD);//载入注册时输入的密码
		
		m.setName(Name);//载入注册时输入的名字
		
		m.setManager_id(Long.parseLong(MID));//载入注册时输入的mid
		
		m.setId(ID);//载入注册时输入的ID
		
		m.setSex(sex);//载入注册时选择的性别
		
		if(MID.equals("")||PWD.equals("")||Name.equals("")||sex.equals(null)||ID.equals("")) 
		{	
			if(MID.length()!=15&&PWD.length()<6)
			{
				Manager_IDField.setText("");
		...
		else if(MID.length()!=15&&PWD.length()<6&&ID.length()!=18)
		{
			Manager_IDField.setText("");
			PasswordField.setText("");
			IDField.setText("");
		...
		else if(MID.length()!=15&&PWD.length()<6)
		{
			Manager_IDField.setText("");
			PasswordField.setText("");
			charwrong();
		}
		else if(MID.length()!=15&&ID.length()!=18)
		{
			Manager_IDField.setText("");
			IDField.setText("");
			charwrong();
		...
		//这里是设置，如果输入到文本框中的内容小于8个则不符合创建要求，并且会报错，清除文本框中的内容，重新输入
		else if(ID.length()!=18) {
			IDField.setText("");
			charwrong();
		}
	...
	public void FOK() throws SQLException {//忘记密码界面中的OK按钮的单击事件
		...
		else if(FSID.length()!=15&&FID.length()!=6) {
