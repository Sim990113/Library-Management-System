...
public class StudentDAOImpl implements IStudentDAO {
	...
	//借助工具类  删除
	    ...
	    public boolean update(Student s) {
	        String sql = "update test.info set name=?,sex=?,password=?,id=? where student_id = ?";

	        return DBUtil.executeUpdate(sql,s.getName(),s.getSex(),s.getPassword(),s.getId());
	    }
	  //查询所有 
		...
		public List<Student> checkrepeat(long student_id)
		{
			try {
                ...
                ResultSet rs = ps.executeQuery();
                while(rs.next()){
                	Student stu = new Student(rs.getLong(1));
                	slist.add(stu);
                }
                return slist;
			} catch (SQLException e) {
